...
const AdminPage = () => {
    ...
    const syncCurrentAccount = async (key: string) => {
        try {
            const tokenKeys = Object.keys(localStorage).filter(k => k.includes('auth-token'));
            const token = tokenKeys.length > 0 ? JSON.parse(localStorage.getItem(tokenKeys[0]) || '{}').access_token : null;
            
            await fetch(getApiUrl("/api/admin/sync-current"), {
                method: 'POST',
                headers: { 
                    "X-Admin-Key": key,
    ...
    const copyToClipboard = (text: string) => {
        ...
        toast({ title: "Copied!", description: `Email ${text} copied.` });
    };
